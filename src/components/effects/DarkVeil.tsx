...
export default function DarkVeil({ fadeStart }: DarkVeilProps) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      try {
        program.uniforms.uTime.value = ((performance.now() - start) / 1000) * 0.6;
    ...
    return () => {
      ...
      geometry.remove();
      program.remove();
      // Avoid forcing context loss to prevent issues on remount
    };
  }, []);

  type VeilStyle = React.CSSProperties & { [key in `--veil-fade-start`]?: string };
  const maskStyle: VeilStyle | undefined = fadeStart ? { "--veil-fade-start": fadeStart } : undefined;
